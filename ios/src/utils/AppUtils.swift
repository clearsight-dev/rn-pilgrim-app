...
class FileUtils {
    ...
    static func copyAssetToDocuments(assetFileName: String, destinationFileName: String) -> Bool {
        ...
        if fileManager.fileExists(atPath: destinationURL.path) {
            NSLog("\(APPTILE_LOG_TAG) :✅ File already exists: \(destinationURL.path)")
            return true
        }
        
        guard let assetPath = Bundle.main.path(forResource: assetFileName, ofType: nil) else {
            NSLog("\(APPTILE_LOG_TAG) :❌ Asset not found: \(assetFileName)")
            return false
        }
        
        do {
            try fileManager.copyItem(atPath: assetPath, toPath: destinationURL.path)
            NSLog("\(APPTILE_LOG_TAG) :✅ File copied successfully from \(assetFileName) to \(destinationURL.path)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error copying file: \(error)")
            return false
        }
    }
    ...
    static func readFileContent(filePath: String) -> String? {
        ...
        guard fileManager.fileExists(atPath: fileURL.path) else {
            NSLog("\(APPTILE_LOG_TAG) :⚠️ File not found: \(filePath)")
            return nil
        }
        
        do {
            let content = try String(contentsOf: fileURL, encoding: .utf8)
            NSLog("\(APPTILE_LOG_TAG) :✅ Read file content: \(filePath)")
            return content
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error reading file: \(error)")
            return nil
        }
    }
    ...
    static func saveFile(data: Data, filePath: String) -> Bool {
        ...
        do {
            ...
            NSLog("\(APPTILE_LOG_TAG) :✅ File saved successfully: \(filePath)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error saving file: \(error)")
            return false
        }
    }
    ...
    static func deleteFile(filePath: String) -> Bool {
        let fileURL = URL(fileURLWithPath: filePath)
        guard fileManager.fileExists(atPath: fileURL.path) else {
            NSLog("\(APPTILE_LOG_TAG) :⚠️ File not found for deletion: \(filePath)")
            return false
        }
        
        do {
            try fileManager.removeItem(at: fileURL)
            NSLog("\(APPTILE_LOG_TAG) :✅ File deleted: \(filePath)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error deleting file: \(error)")
            return false
        }
    }
    ...
    static func moveFile(sourcePath: String, destinationPath: String) -> Bool {
        ...
        guard fileManager.fileExists(atPath: sourceURL.path) else {
            NSLog("\(APPTILE_LOG_TAG) :⚠️ Source file not found: \(sourcePath)")
            return false
        }
        
        do {
            try fileManager.moveItem(at: sourceURL, to: destinationURL)
            NSLog("\(APPTILE_LOG_TAG) :✅ File moved from \(sourcePath) to \(destinationPath)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error moving file: \(error)")
            return false
        }
    }
    ...
    static func unzip(zipFilePath: String, destinationPath: String) -> Bool {
        ...
        do {
            ...
            NSLog("\(APPTILE_LOG_TAG) :✅ Unzipped successfully: \(zipFilePath) -> \(destinationPath)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error unzipping file: \(error)")
            return false
        }
    }
    ...
    static func verifyFileIntegrity(filePath: String, expectedHash: String) -> Bool {
        ...
        guard fileManager.fileExists(atPath: fileURL.path),
            ...
            NSLog("\(APPTILE_LOG_TAG) :⚠️ File not found for hashing: \(filePath)")
            return false
        }
        ...
        NSLog("\(APPTILE_LOG_TAG) :🔍 Computed Hash: \(computedHash)")
        NSLog("\(APPTILE_LOG_TAG) :🔍 Expected Hash: \(expectedHash)")
...
//        return computedHash.lowercased() == expectedHash.lowercased()
    ...
    static func copyDirectoryContents(sourcePath: String, destinationPath: String) {
        ...
        do {
            ...
            NSLog("\(APPTILE_LOG_TAG) :✅ Copied contents from \(sourcePath) to \(destinationPath)")
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Error copying directory contents: \(error)")
        }
    }
    ...
    static func downloadFile(from url: URL, to destinationFileName: String) async -> Bool {
        ...
        do {
            ...
            NSLog("\(APPTILE_LOG_TAG) :✅ File downloaded successfully: \(destinationURL.path)")
            return true
        } catch {
            NSLog("\(APPTILE_LOG_TAG) :❌ Download failed: \(error)")
            return false
        }
    }
}
