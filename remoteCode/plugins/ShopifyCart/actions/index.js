...
import {makeInputParamsResolver} from '../source/widget';
...
class CartActions {
  getCart = model => model?.get(CART_OBJECT_KEY);
  swapCart;

  // Local Storage Methods
  getCartFromLS = (config) => config.config.get(CART_KEY_FOR_LOCAL_STORAGE_KEY);

  storeCartObjectLocally = async (config, cartObject) => {
  ...
  removeCartObjectLocally = async (config) => {
    const cartLocalStorageKey = this.getCartFromLS(config);
    await LocalStorage.removeKey(cartLocalStorageKey);
  };

  // Caching Methods
  cacheCartLineItem = async (
    dispatch,
    config,
    model,
    selector,
    ...
    try {
      ...
      const transformedData = await this.fetchVariantData(appConfig, appModel, model, [merchandiseId]);
      const variantData = _.head(transformedData);
      if (variantData) {
        cartLineCache = {...cartLineCache, [merchandiseId]: variantData};
        dispatch(
          modelUpdateAction(
            [{selector: selector.concat([CART_LINE_CACHE_KEY]), newValue: cartLineCache}],
            undefined,
            true,
          ),
        );
  ...
  refreshCartLineCache = async (
    ...
    if (isCacheEnabled) {
      const cartLineCache = this.getCartLineCache(model, appConfig, appModel);
  ...
  getCartLineCache = async (model, appConfig, appModel) => {
    ...
    if (!_.isEmpty(lineItems)) {
      const variantIds = _.uniq(_.map(lineItems, entry => entry?.merchandiseId));
      const transformedData = await this.fetchVariantData(appConfig, appModel, model, variantIds);
  ...
  async fetchVariantData(appConfig, appModel, model, variantIds) {
    ...
    const response = await this.queryExecutor(appConfig, appModel, model, getVariantsQuery, queryInput);
    return response?.transformedData;
  }

  // Verification Methods
  validateCartLineItemsRules = async (
    dispatch,
    config,
    model,
    selector,
    ...
    const OrderLimiterDSModel = appModel?.getModelValue([orderLimiterDataSourceId]);
    const isCartSatisfyOrderLimit = OrderLimiterDSModel?.get('isCartSatisfyOrderLimitV2');
  ...
  // Utility Methods
  getCartLineItemsMap = cartObject => {
    const cartLineItems = _.get(cartObject, 'lines', []);
    const cartLineMap = {};

    cartLineItems.forEach(entry => {
      const merchandiseId = _.get(entry, 'variant.id');
      const currentItemQuantity = _.get(cartLineMap, [merchandiseId, 'quantity'], 0);
      _.set(cartLineMap, merchandiseId, {quantity: currentItemQuantity + _.get(entry, 'quantity', 0)});
    });

    return cartLineMap;
  };

  getMatchingLineItem = (model, merchandiseId, sellingPlanId, cartLineId) => {
    const currentCart = this.getCart(model);
    const cartLineItems = _.get(currentCart, 'lines', []);

    if (!_.isEmpty(cartLineId)) {
      return _.find(cartLineItems, entry => entry?.cartLineId === cartLineId) ?? {};
    }
  ...
  addLineItemToSyncManager = (model, merchandiseId, cartLineId) => {
  ...
  removeLineItemFromSyncManager = (model, merchandiseId) => {
    const syncManager = model?.get(SYNCING_LINE_ITEM_KEY);
    const recreateSyncManager = Object.assign({}, syncManager ?? {});
  ...
  // Loading Methods
  startLoading(dispatch, selector, model) {
    const modelUpdates = [{selector: selector.concat([CART_SYNC_KEY]), newValue: true}];
    dispatch(modelUpdateAction(modelUpdates, undefined, true));
  }

  startLoadingAndAddItemToSync(dispatch, selector, model, merchandiseId) {
  ...
  completeLoading = (dispatch, selector, model) => {
    const modelUpdates = [{selector: selector.concat([CART_SYNC_KEY]), newValue: false}];
    dispatch(modelUpdateAction(modelUpdates, undefined, true));
  };

  completeLoadingAndRemoveItemFromSync = (dispatch, selector, model, merchandiseId) => {
    const syncingLineItems = this.removeLineItemFromSyncManager(model, merchandiseId);
    const modelUpdates = [
      {selector: selector.concat([CART_SYNC_KEY]), newValue: false},
      {selector: selector.concat([SYNCING_LINE_ITEM_KEY]), newValue: syncingLineItems},
    ];
    dispatch(modelUpdateAction(modelUpdates, undefined, true));
  };

  // Model Update Methods
  finalizeModelUpdates(dispatch, selector, model, cartObject) {
    const cartLineItemsMap = this.getCartLineItemsMap(cartObject);

    const modelUpdates = [
      {selector: selector.concat([CART_LINE_ITEMS_MAP_KEY]), newValue: cartLineItemsMap},
      {selector: selector.concat([CART_SYNC_KEY]), newValue: false},
    ];

    // This prevent clearing of existing cart due to some unexpected issues.
    if (!_.isEmpty(cartObject)) {
      modelUpdates.push({selector: selector.concat([CART_OBJECT_KEY]), newValue: cartObject});
    }

    dispatch(modelUpdateAction(modelUpdates, undefined, true));
  }

  finalizeModelUpdatesAndRemoveItemFromSync(dispatch, selector, model, merchandiseId, cartObject) {
    const cartLineItemsMap = this.getCartLineItemsMap(cartObject);
    const modelUpdates = [
      {selector: selector.concat([CART_LINE_ITEMS_MAP_KEY]), newValue: cartLineItemsMap},
      {
  ...
  executeCartQuery = async (model, query, inputs, appConfig, appModel, _cartId) => {
    try {
      let result;
      ...
      if (!_.isEmpty(result?.transformedError) && model?.get('showShopifyCartErrorMessage')) {
        toast.show(_.get(_.head(result?.transformedError), 'message'), {
          type: 'error',
          placement: 'bottom',
          duration: 2000,
  ...
  queryExecutor = async (
    ...
    const tResponse = processShopifyGraphqlQueryResponse(response, querySchema, shopConfig, pluginModel);
    return tResponse;
  };

  // Post Actions
  postCartActions = async (
    dispatch,
    config,
    model,
    selector,
    ...
    const handleError = _error => {
      dispatch(modelUpdateAction([{selector: selector.concat([CART_SYNC_KEY]), newValue: false}], undefined, true));
    };
    ...
    try {
      ...
      const adjustCartGifts = freeGiftDSModel?.get('attachFreebieToCartLineItems');
      const isCartPreparing = model?.get(CART_PREPARE_KEY);
      const modelUpdates = [];
      ...
      if (adjustCartGifts) {
        ...
        const {giftsToAdd, giftsToRemove} = adjustCartGifts(freeGiftDSModel, incomingCartObject, cartLineCache);
        ...
        if (!_.isEmpty(postProcessedCartObject)) {
          const cartLineItemsMap = this.getCartLineItemsMap(postProcessedCartObject);
          ...
          modelUpdates.push({selector: selector.concat([CART_OBJECT_KEY]), newValue: postProcessedCartObject});
      ...
      this.preloadCheckout(
        model,
        !_.isEmpty(postProcessedCartObject) ? postProcessedCartObject : incomingCartObject,
        forceCheckoutPreload,
      );
    } catch (err) {
      handleError(err);
    }
  ...
  // Operation handlers
  addLineItem = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;

      const {attributes, quantity, merchandiseId, sellingPlanId, itemPrice} = params;
      const lineItems = [];
      ...
      this.startLoadingAndAddItemToSync(dispatch, selector, model, merchandiseId);
      ...
      this.finalizeModelUpdatesAndRemoveItemFromSync(dispatch, selector, model, merchandiseId, cartObject);
      this.storeCartObjectLocally(config, cartObject);
      this.postCartActions(dispatch, config, model, selector, params, appConfig, appModel, cartObject, cartLineCache);
    } catch (err) {
      const {merchandiseId} = params;
      this.completeLoadingAndRemoveItemFromSync(dispatch, selector, model, merchandiseId);
      console.error(`Error executing cart addLineItem`, err);
    }
  };

  updateLineItem = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;

      // Method to be used for value manipulation. Can be either 'INCREMENT' or 'DECREMENT'.
      const {attributes, quantity, merchandiseId, sellingPlanId, itemPrice, id, method = 'INCREMENT'} = params;
      ...
      const matchingLineItem = this.getMatchingLineItem(model, merchandiseId, sellingPlanId, id);
      if (_.isEmpty(matchingLineItem)) {
        console.warn(`No matching line item found for merchandiseId: ${merchandiseId}`);
        return;
      }
      ...
      lineItems.push({
        ...
        quantity:
          method === 'INCREMENT'
            ? _.get(matchingLineItem, 'quantity') + (quantity || defaultQuantity)
            : _.get(matchingLineItem, 'quantity') - (quantity || defaultQuantity),
        attributes: _.isEmpty(attributes) ? [] : attributes,
      });
  ...
  increaseLineItemQuantity = async (...args) => {
    try {
      const [dispatch, _config, model, selector, params, _appConfig, _appModel] = args;
  ...
  removeLineItem = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
      ...
      this.finalizeModelUpdatesAndRemoveItemFromSync(dispatch, selector, model, merchandiseId, cartObject);
      this.storeCartObjectLocally(config, cartObject);
      this.postCartActions(dispatch, config, model, selector, params, appConfig, appModel, cartObject, undefined);
    } catch (err) {
      const {merchandiseId} = params;
      this.completeLoadingAndRemoveItemFromSync(dispatch, selector, model, merchandiseId);
  ...
  clearCart = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    const isCartModifiable = this.isCartModifiable(model);
    if (!isCartModifiable) return;
  ...
  refreshCart = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
      ...
      this.startLoading(dispatch, selector, model);
      const {transformedData} = await this.executeCartQuery(model, 'GET_CART', {}, appConfig, appModel);
      cartObject = transformedData;
  ...
  updateDiscounts = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
      ...
      this.finalizeModelUpdates(dispatch, selector, model, cartObject);
      this.storeCartObjectLocally(config, cartObject);
      this.postCartActions(dispatch, config, model, selector, params, appConfig, appModel, cartObject, undefined, true);
    } catch (err) {
      this.completeLoading(dispatch, selector, model);
  ...
  updateAttributes = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
  ...
  updateNote = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
  ...
  updateBuyerIdentity = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    try {
      const isCartModifiable = this.isCartModifiable(model);
      if (!isCartModifiable) return;
  ...
  initiateCheckout = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    const isCheckoutSheetEnabled = model?.get(ENABLE_CHECKOUT_SHEET_KEY);
    const isPreCheckoutValidationEnabled = model?.get(ENABLE_PRE_CHECKOUT_VALIDATION);

    if (isPreCheckoutValidationEnabled) {
      const isCartValid = await this.validateCheckout(dispatch, config, model, selector, params, appConfig, appModel);
      if (!isCartValid) {
  ...
  validateCheckout = async (dispatch, config, model, selector, params, appConfig, appModel) => {
    const cartObject = this.getCart(model);
    const cartLineCache = await this.getCartLineCache(model, appConfig, appModel);
