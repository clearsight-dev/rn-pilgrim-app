...
const CustomHeader = forwardRef(
  ({navigation, topInset, numCartItems, showBackbutton}, ref) => {
    // const locals = useRef({
    ...
    const handleHeaderLeft = () => {
      ...
      if (navigation.canGoBack()) {
        ...
        console.warn(
          'Triggered back navigation on a screen that cannot go back',
        );
      }
    };
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <Icon
              ...
              name={showBackbutton ? 'keyboard-backspace' : 'search'}
              size={24}
              color="#333"
            />
          </TouchableOpacity>
...
function CustomHeaderSmart({route, navigation}) {
  ...
  useEffect(() => {
    ...
    if (
      isSearchbarVisible &&
      pilgrimGlobals.homePageScrolledDown === true &&
      onHome
    ) {
...
export default function createCustomStackNavWithHeader(
  ...
  let NavScreenOptions = {
    ...
    header: ({route, navigation, navConfig}) => (
      <CustomHeaderSmart route={route} navigation={navigation} />
    ),
  };
